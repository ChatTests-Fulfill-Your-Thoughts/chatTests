...
export class LoreModel {
  ...
  public addQuestion(item: LoreQuestionInfo): void {
    ...
    this._data.list = list?.map((val) => {
      if (val.uid === current) {
        ...
        val.questions = (val.questions || []).concat(item.topic);
      }
      console.log(val);
      return val;
    });
  ...
  public get newQuestionPrompt(): ChatMessage[] {
    ...
    const exclude: string = questions?.reduce(
      (prev, curr) => prev + curr.topic,
      ""
    );
    return [
      {
        role: "system",
        content: `你是一个备考专家，需要为用户提供出单选题的服务。
        请以JSON格式提供你的输出，不要有其他任何无关回复。
        JSON格式包含以下键：topic(题干)，A(选项内容)，B(选项内容)，C(选项内容)，D(选项内容)correct(正确选项数字)，analyze(解析)。
        举例如下：
        {
